...
func _on_playmode_state_changed(current_playmode_state: PlayModeState) -> void:
	if current_playmode_state is ExplorationMode and _current_mode != exploration_mode and exploration_mode:
		set_mode(exploration_mode)
	elif current_playmode_state is FightMode and _current_mode != combat_mode and combat_mode:
		set_mode(combat_mode)
	elif current_playmode_state is BuildingMode and _current_mode != building_mode and building_mode:
