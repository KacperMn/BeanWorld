...
func set_npc_location_data(position: Vector3, _entity: Node) -> void:
	npc_position = position
	entity = _entity

func wander(delta: float) -> void:
	if target_location != null and Vector2(npc_position.x - target_location.x, npc_position.z - target_location.z).length() < 2.0:
